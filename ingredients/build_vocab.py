...
def build_vocab_recipe1m(args):
    print ("Loading data...")
    dets = json.load(open(os.path.join('ingrs_path.json'), 'r'))
    layer1 = json.load(open(os.path.join('layer1_path.json'), 'r'))
    layer2 = json.load(open(os.path.join('layer2_path.json'), 'r'))
    ...
    ingrs_file = 'allingrs_count.pkl'
    instrs_file = 'allwords_count.pkl'
    ...
    if os.path.exists(ingrs_file) and os.path.exists(instrs_file) and not args.forcegen:
        ...
        counter_ingrs = pickle.load(open('allingrs_count.pkl', 'rb'))
        counter_toks = pickle.load(open('allwords_count.pkl', 'rb'))
    else:
...
def main(args):
    ...
    with open(os.path.join('vocab_ingrs.pkl'), 'wb') as f:
        pickle.dump(vocab_ingrs, f)

    for split in dataset.keys():
        with open(os.path.join('recipe1m_' + split + '.pkl'), 'wb') as f:
